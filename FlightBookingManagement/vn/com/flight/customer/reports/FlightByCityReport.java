package vn.com.flight.customer.reports;

import java.util.Collection;
import java.util.Map;

import domainapp.basics.core.dodm.dsm.DSMBasic;
import domainapp.basics.core.dodm.qrm.QRM;
import domainapp.basics.exceptions.DataSourceException;
import domainapp.basics.exceptions.NotPossibleException;
import domainapp.basics.model.Oid;
import domainapp.basics.model.meta.AttrRef;
import domainapp.basics.model.meta.DAssoc;
import domainapp.basics.model.meta.DAssoc.AssocEndType;
import domainapp.basics.model.meta.DAssoc.AssocType;
import domainapp.basics.model.meta.DAssoc.Associate;
import domainapp.basics.model.meta.DAttr;
import domainapp.basics.model.meta.DAttr.Type;
import domainapp.basics.model.meta.DClass;
import domainapp.basics.model.meta.DOpt;
import domainapp.basics.model.meta.MetaConstants;
import domainapp.basics.model.meta.Select;
import domainapp.basics.model.query.Expression.Op;
import domainapp.basics.model.query.Query;
import domainapp.basics.model.query.QueryToolKit;
import domainapp.basics.modules.report.model.meta.Output;
import vn.com.flight.customerManagement.City;
import vn.com.flight.customerManagement.Customer;
import vn.com.flight.flightManagement.Flight;

/**
 * @overview 
 * 	Represent a report about flight by city whose view is expressed by a join query.
 * 
 * @author G3-SS207
 *
 * @version 5.3
 */
@DClass(schema="FLIGHT",serialisable=false)
public class FlightByCityReport {
  @DAttr(name = "id", id = true, auto = true, type = Type.Integer, length = 5, optional = false, mutable = false)
  private int id;
  private static int idCounter = 0;

  /**input: city name */
  @DAttr(name = "cityName", type = Type.String, length = 30, optional = false)
  private String cityName;
  
  /**output: students whose names match {@link #cityName} */
  @DAttr(name="flight",type=Type.Collection,optional=false, mutable=false,
      serialisable=false,filter=@Select(clazz=Flight.class, 
      attributes={Flight.A_id, Flight.A_DepartureDate, Flight.A_ArrivalDate, Flight.A_DepartDestination, 
    	Flight.A_ArrivalDestination, Flight.A_Airplane,Flight.A_Airport,Flight.A_Airline,Flight.A_rptFlightByCity})
      ,derivedFrom={"cityName"})
  @DAssoc(ascName="flight-by-cityName-report-has-flight",role="report",
      ascType=AssocType.One2Many,endType=AssocEndType.One,
    associate=@Associate(type=Flight.class,cardMin=0,cardMax=MetaConstants.CARD_MORE
    ))
  @Output
  private Collection<Flight> flight;

  /**output: number of flight found (if any), derived from {@link #flight} */
  @DAttr(name = "numFlights", type = Type.Integer, length = 20, auto=true, mutable=false)
  @Output
  private int numFlights;
  
  /**
   * @effects 
   *  initialise this with <tt>cityName</tt> and use {@link QRM} to retrieve from data source 
   *  all {@link Customer} whose addresses match {@link City}, whose names match <tt>cityName</tt>.
   *  initialise {@link #flight} with the result if any.
   *  
   *  <p>throws NotPossibleException if failed to generate data source query; 
   *  DataSourceException if fails to read from the data source
   * 
   */
  @DOpt(type=DOpt.Type.ObjectFormConstructor)
  @DOpt(type=DOpt.Type.RequiredConstructor)
  public FlightByCityReport(@AttrRef("cityName") String name) throws NotPossibleException, DataSourceException 
  {
    this.id=++idCounter;
    
    this.cityName = name;
    
    doReportQuery();
  }
  
  /**
   * @effects return cityName
   */
  public String getCityName() 
  {
    return cityName;
  }

  /**
   * @effects <pre>
   *  set this.name = cityName
   *  if cityName is changed
   *    invoke {@link #doReportQuery()} to update the output attribute value
   *    throws NotPossibleException if failed to generate data source query; 
   *    DataSourceException if fails to read from the data source.
   *  </pre>
   */
  public void setCityName(String name) throws NotPossibleException, DataSourceException 
  {
//    boolean doReportQuery = (cityName != null && !cityName.equals(this.name));
    
    this.cityName = name;
    
    // DONOT invoke this here if there are > 1 input attributes!
    doReportQuery();
  }

  /**
   * This method is invoked when the report input has be set by the user. 
   * 
   * @effects <pre>
   *   formulate the object query
   *   execute the query to retrieve from the data source the domain objects that satisfy it 
   *   update the output attributes accordingly.
   *  
   *  <p>throws NotPossibleException if failed to generate data source query; 
   *  DataSourceException if fails to read from the data source. </pre>
   */
  @DOpt(type=DOpt.Type.DerivedAttributeUpdater)
  @AttrRef("flight")
  public void doReportQuery() throws NotPossibleException, DataSourceException 
  {
    // the query manager instance
    
    QRM qrm = QRM.getInstance();
    
    // create a query to look up Student from the data source
    // and then populate the output attribute (students) with the result
    DSMBasic dsm = qrm.getDsm();
    
    ////TODO: create a 2-way join query
    Query q = QueryToolKit.createSimpleJoinQuery(dsm, Flight.class, City.class,  
        Flight.A_DepartDestination, 
        City.A_name, 
        Op.MATCH, 
        "%"+cityName+"%");
    
    Map<Oid, Flight> result = qrm.getDom().retrieveObjects(Flight.class, q);
    
//    Query q1 = QueryToolKit.createSimpleJoinQuery(dsm, Flight.class, City.class,  
//            Flight.A_ArrivalDestination, 
//            City.A_name, 
//            Op.MATCH, 
//            "%"+cityName+"%");
//        
//        Map<Oid, Flight> result1 = qrm.getDom().retrieveObjects(Flight.class, q);
//    
    if (result != null) {
      // update the main output data 
      flight = result.values();
      
      // update other output (if any)
      numFlights = flight.size();
    } else {
      // no data found: reset output
      resetOutput();
    }
  }

  /**
   * @effects 
   *  reset all output attributes to their initial values
   */
  private void resetOutput() 
  {
    flight = null;
    numFlights = 0;
  }

  /**
   * A link-adder method for {@link #flight}, required for the object form to function.
   * However, this method is empty because customer have already be recorded in the attribute {@link #flight}.
   */
  @DOpt(type=DOpt.Type.LinkAdder)
  public boolean addFlight(Collection<Flight> flight) 
  {
    // do nothing
	  
    return false;
  }
  
  /**
   * @effects return flight
   */
  public Collection<Flight> getFlight() 
  {
    return flight;
  }
  
  /**
   * @effects return numCustomer
   */
  public int getNumFlights() 
  {
    return numFlights;
  }

  /**
   * @effects return id
   */
  public int getId() 
  {
    return id;
  }

  /* (non-Javadoc)
   * @see java.lang.Object#hashCode()
   */
  /**
   * @effects 
   * 
   * @version 
   */
  @Override
  public int hashCode() 
  {
    final int prime = 31;
    int result = 1;
    result = prime * result + id;
    return result;
  }

  /* (non-Javadoc)
   * @see java.lang.Object#equals(java.lang.Object)
   */
  /**
   * @effects 
   * 
   * @version 
   */
  @Override
  public boolean equals(Object obj) 
  {
    if (this == obj)
      return true;
    if (obj == null)
      return false;
    if (getClass() != obj.getClass())
      return false;
    FlightByCityReport other = (FlightByCityReport) obj;
    if (id != other.id)
      return false;
    return true;
  }

  /**
   * @effects 
   * 
   * @version 
   */
  @Override
  public String toString() 
  {
    return this.getClass().getSimpleName()+ " (" + id + ", " + cityName + ")";
  }
}
